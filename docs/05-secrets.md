...
(optional, defaults to `22`). `private key` / `public key` are the category's
built-in fields - 1Password derives `public key` automatically, whether the
key was generated by `op` or pasted in.

Create one with a freshly generated Ed25519 key:

```bash
op item create --category="SSH Key" --title="proxmox-vm" --vault=dotfiles \
  --tags=ssh-host --ssh-generate-key=ed25519 \
  'hostname[text]=10.0.0.10' 'user[text]=hermes' 'port[text]=22'
```

Then add `ssh-ed25519 AAAA...` (`op read "op://dotfiles/proxmox-vm/public key"`)
to that server's `~/.ssh/authorized_keys`. To register a key that already
exists instead of generating a new one, drop `--ssh-generate-key` and pass the
private key contents as an assignment (`private_key[sshkey]=$(cat id_ed25519)`)
or through a template - see `op item create --help`.

Edit an existing item's fields the same way, in place:

```bash
op item edit proxmox-vm --vault=dotfiles 'hostname[text]=10.0.0.11'
```

The task only ever touches the block between its own
`# --- BEGIN op ssh:sync ... # --- END op ssh:sync ---` markers in
`config.local`, so it's safe to re-run and safe to hand-edit anything outside
that block. `~/.ssh/config` (Include order above) is never written to -
`config.local` is already the untracked, host-specific file this repo
designates for exactly this.

**Why `IdentityFile` + `IdentitiesOnly yes` per host.** sshd's default
`MaxAuthTries` is 6, and each key ssh-agent offers counts as one try - with
more than a handful of keys loaded, ssh can exhaust the server's attempt
budget and get disconnected ("too many authentication failures") before it
ever offers the right key. `ssh:sync` writes each host's public key (not
sensitive) to `~/.ssh/1p-hosts/<host>.pub` and pins the host block to it, so
ssh only ever offers that one key for that host, regardless of how many other
keys are sitting in the agent.
