---
name: omp-tuning
description: Use when changing this OMP setup itself - editing configs/omp/config.yml, models.yml, keybindings.yml, adding or overriding a subagent under configs/omp/agents/, writing a new skill, or deciding whether guidance belongs in AGENTS.md, RULES.md, WATCHDOG.md or APPEND_SYSTEM.md, plus how to verify the change took effect.
---
...
The tracked source of truth is `configs/omp/`, copied into the image at
...
|---|---|---|
| `configs/omp/config.yml` | `~/.omp/agent/config.yml` | every setting: `modelRoles`, `task.agentModelOverrides`, `tools.approvalMode`, `memory`, `theme`, ... |
| `configs/omp/models.yml` | `~/.omp/agent/models.yml` | custom providers/models, aliases, per-model overrides. Not settings. |
| `configs/omp/keybindings.yml` | `~/.omp/agent/keybindings.yml` | key remaps only. Flat mapping of action id -> chord; there is **no** `keybindings` block in `config.yml`. |
| `configs/omp/agents/<name>.md` | `~/.omp/agent/agents/<name>.md` | one subagent definition per file (frontmatter `name` + `description` required, body = its system prompt). |
| `configs/omp/skills/<name>/SKILL.md` | `~/.omp/agent/skills/<name>/SKILL.md` | procedures like this one. |
...
`configs/omp/skills/` invisible to the model. Always prefer `APPEND_SYSTEM.md`.
...
Creating `configs/omp/agents/scout.md` therefore does not tweak `scout` - it
...
```
configs/omp/skills/<skill-name>/SKILL.md
```

`configs/omp/skills/group/<skill>/SKILL.md` is **not discovered**. Frontmatter
...
`configs/omp/` - that would violate the no-secrets-in-the-repo invariant
(`skill://add-secret`).
...
`configs/omp/` is inside the image, so a change is not real until:
